...
Rcpp::List query_bgen13(SEXP bgenR_in, SEXP seek_in)
{
  ...
  return(Rcpp::List::create(Named("SNPID")         = Rcpp::String(snpID2),
                            ...
                            Named("Position")      = physpos,
                            Named("Alleles")       = LKnum,
                            Named("Allele1")       = Rcpp::String(allele12),
                            Named("Allele2")       = Rcpp::String(allele22),
                            Named("AF")            = AF,
  ...
  for (uint i = 0; i < m; i++) {
    ...
    if (l == 2) {
      ...
      if (LKnum != 2U) {
        ...
        vecSNPID[i] = Rcpp::String(snpID2);
        vecRSID[i]  = Rcpp::String(rsID2);
        vecCHR[i]   = Rcpp::String(chr2);
        vecPOS[i]   = physpos;
        vecLK[i]    = LKnum;
        vecA1[i]    = NA_STRING;
        vecA2[i]    = NA_STRING;
    ...
    vecSNPID[i] = Rcpp::String(snpID2),
      ...
      vecPOS[i]   = physpos;
    vecLK[i]    = LKnum;
    vecA1[i]    = Rcpp::String(allele12);
    vecA2[i]    = Rcpp::String(allele22);
  }
  fseek(fstream, offset + 4, SEEK_SET);
